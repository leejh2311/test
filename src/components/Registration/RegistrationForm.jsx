...
const RegistrationForm = () => {
  ...
  const handleAddItem = () => {
    ...
    const updatedCategoryItems = [
      ...(itemsByCategory[currentCategory] || []),
      newItem,
    ];
    setItemsByCategory({
      ...itemsByCategory,
      [currentCategory]: updatedCategoryItems,
    });
  };

  const handleRemoveItem = (category, index) => {
    const updatedCategoryItems = itemsByCategory[category].filter(
      (_, idx) => idx !== index
    );
    setItemsByCategory({
      ...itemsByCategory,
          ...
          itemsByCategory[categories[selectedTab]].map((item, index) => (
            <Grid container spacing={2} key={index} alignItems="center">
              ...
              <Grid item xs={12} sm={2}>
                <IconButton
                  onClick={() =>
                    handleRemoveItem(categories[selectedTab], index)
                  }
                  color="error"
                >
                  <DeleteIcon />
